...
export default function DiscoverBlock({ text, id, data, isFetching, imagesKey = 'images' }) {
  const renderContent = () => {
    if (isFetching) {
      return (
        <div className="discover-block__spinner">
          <FontAwesomeIcon icon={faSpinner} spin size="6x" />
        </div>
      )
    }

    return data.map(({ [imagesKey]: images, name }) => <DiscoverItem key={name} images={images} name={name} />)
  }
